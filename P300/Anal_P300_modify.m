...
for ind = 1:nChoice
    R1(:,:) = EEG(:,iChoices(ind)+offset);
    Respond1 = filtfilt(b,a,R1')';
    EEG1 = Respond1(:,:);
    MeanResponse = MeanResponse+EEG1(:,:);
end
...
for ind = 1:nNotChoice
    R2(:,:) = EEG(:,iNotChoices(ind)+offset);
    NotResponse1 = filtfilt(b,a,R2')';
    EEG2 = NotResponse1(:,:);
    MeanNotResponse = MeanNotResponse+EEG2(:,:);
end
